...
const Header = () => {
  ...
  return (
    <header className="fixed top-0 left-0 w-screen backdrop-blur-3xl shadow-md bg-white lg:h-[100px] z-5 py-2 lmd:pb-0">
      <nav className="app-padding flex justify-between items-center">
        ...
        <div
          className={`flex flex-col flex-1 lmd:flex-row lmd:justify-between lmd:items-center gap-6 w-[90%] lmd:w-max fixed h-lvh lmd:h-auto top-0 right-0 lmd:relative p-5 lmd:p-[auto] ${isOpen ? 'translate-x-0 bg-white text-primaryText' : 'translate-x-[100%]'} lmd:translate-x-0 z-20 transition-transform `}
        >
          <button className="flex lmd:hidden mb-10" onClick={handleIsOpen}>
            <SvgIcon name="close" className="w-4 h-4" />
          </button>

          <div className="hidden lmd:block max-w-[600px] mx-auto flex-1">
            <Search iconButton />
          </div>
